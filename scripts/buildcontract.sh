...
case $NETWORK in
  ...
  testnet)
    ...
    ADDR_ADMIN="inj1tdjflhg75ddkc98wpf4aev5clw4fseuqauq3ys"
    ;;
  mainnet)
...
TXFLAG=" $NODECHAIN --gas-prices 500000000$DENOM  --gas 20000000 --gas-adjustment 1.3"
...
Upload() {
    ...
    UPLOADTX=$(injectived tx wasm store ./release/*.wasm $WALLET $TXFLAG --output json -y | jq -r '.txhash')
    ...
    do 
        sleep 3
        CODE_ID=$(injectived query tx $UPLOADTX $NODECHAIN --output json | jq -r '.logs[0].events[-1].attributes[2].value')
    done
...
InstantiateIncentive() {
    CODE_INCENTIVE=$(cat $CODE_DIR"cw721")
    
    TXHASH=$(injectived tx wasm instantiate $CODE_INCENTIVE '{"owner":"sei1qfh0vm2m7p9acvlmy0mwkez6j69scpuccassu4", "max_tokens":1, "unit_price":100000000, "name":"SYNTH", "symbol":"SYNTH", "token_code_id":1819,cw20_address:"sei1qfh0vm2m7p9acvlmy0mwkez6j69scpuccassu4","token_uri":"https://ipfs.io/ipfs/Q","extension":"" }' --label "cw721$CODE_INCENTIVE" --admin $ADDR_ADMIN $WALLET $TXFLAG -y --output json | jq -r '.txhash')
    ...
    do
        sleep 3
        CONTRACT_ADDR=$(injectived query tx $TXHASH $NODECHAIN --output json | jq -r '.logs[0].events[0].attributes[0].value')
    done
    echo $CONTRACT_ADDR
...
ClaimFlip() {
    ...
    echo $(injectived tx wasm execute $CONTRACT_INCENTIVE '{"flip": {"level": 0}}' --amount 2000000ustars --from st $TXFLAG -y)
}

ClaimRPS() {
    CONTRACT_INCENTIVE=$(cat $ADDRESS_DIR"incentive")
    echo $(injectived tx wasm execute $CONTRACT_INCENTIVE '{"rps": {"level": 0}}' --amount 2000000ustars --from st $TXFLAG -y)
}

WithDraw() {
    CONTRACT_INCENTIVE=$(cat $ADDRESS_DIR"incentive")
    echo $(injectived tx wasm execute $CONTRACT_INCENTIVE '{"withdraw": {"amount": "5000000"}}' --from testnet-key $TXFLAG -y)
}
